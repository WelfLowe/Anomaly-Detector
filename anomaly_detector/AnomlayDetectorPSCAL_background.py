...
class AnomalyDetectorPSCAL_background(AnomalyDetector):

    def get_name(self) -> str:
        return "PSCALbackground"
    ...
    def train_eval(self, r, s) -> List[float]:
        ...
        def train_one_epoch_vanilla(model,
                                    optimizer,
                                    dataloader,
                                    epoch,
                                    log_file=f"logs/training_log_{r}_{s}.csv"):
            model.train()
        ...
        def train_vanilla(data_holder, epochs, n_coupling_blocks, clamp_alpha,
            ...
            for epoch in range(epochs):
                ...
                train_one_epoch_vanilla(model, optimizer, dataloader, epoch)
            return model
        ...
        data_holder2 = utl.DataHolderLabeled(data_norm, self.labels)

        model = train(data_holder, c.epochs, c.std_cutoff, c.n_coupling_blocks,
                      c.clamp_alpha, c.fc_internal, c.dropout, c.learning_rate,
                      c.batch_size)
        model2 = train_vanilla(data_holder2, c.epochs, c.n_coupling_blocks,
        ...
        torch.save(model, 'pscal.pth')
        torch.save(model2, 'pscal_background.pth')
        #
        scores_val = np.array(evaluate(model, dataloader))
        scores_val_vanilla = np.array(evaluate(model2, dataloader))

        scores_val = scores_val - scores_val_vanilla
