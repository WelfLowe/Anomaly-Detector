...
def train_one_epoch(model, optimizer, dataloader, std_cutoff):
    ...
    for data, index in dataloader:
        ...
        if len(new_outliers) > 0:
            indices_of_outliers = list(index[
                new_outliers.detach().cpu().numpy()].detach().cpu().numpy())
...
def check_for_inliers(model, dataloader, mean, std, std_cutoff):
    ...
    for data, index in dataloader:
        ...
        if len(new_inliers) > 0:
            indices_of_inliers = list(index[
                new_inliers.detach().cpu().numpy()].detach().cpu().numpy())
...
def train(data_holder, epochs, std_cutoff, n_coupling_blocks, clamp_alpha,
    ...
    for _ in range(epochs):
        ...
        if last_mean and data_holder.get_n_bad() > 0:
            ...
            inliers = check_for_inliers(model, bad_data_dataloader, last_mean,
                                        last_std, std_cutoff)
...
model = train(data_holder, 200, c.std_cutoff, c.n_coupling_blocks,
              c.clamp_alpha, c.fc_internal, c.dropout, c.learning_rate,
              c.batch_size)

torch.save(model, "giustina2.pth")
